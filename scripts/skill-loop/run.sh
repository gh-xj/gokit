...
PENDING_FILE="${WORKTREE_DIR}/docs/loop/pending-improvements.md"
LOG_FILE="${WORKTREE_DIR}/docs/loop/improvement-log.md"
if [[ -f "${PENDING_FILE}" ]]; then
  NEXT_ITEM="$(grep -E '^- \[ \] ' "${PENDING_FILE}" | head -n1 || true)"
  if [[ -n "${NEXT_ITEM}" ]]; then
    NEXT_TEXT="${NEXT_ITEM#- [ ] }"
    log "applying pending improvement: ${NEXT_TEXT}"
    awk -v target="${NEXT_TEXT}" '
      BEGIN { done=0 }
      {
        if (!done && $0 == "- [ ] " target) {
          print "- [x] " target
    ...
    printf '%s - %s\n' "$(date -u +%Y-%m-%dT%H:%M:%SZ)" "${NEXT_TEXT}" >> "${LOG_FILE}"
    git add "${PENDING_FILE}" "${LOG_FILE}"
    git commit -m "chore: apply pending improvement from evolve loop" >/dev/null || true
  fi
fi

COMMITS_AHEAD="$(git rev-list --count "${SOURCE_REF}..HEAD")"
if [[ "${COMMITS_AHEAD}" == "0" ]]; then
  log "no changes produced by loop; skipping push and PR creation"
  log "done"
  exit 0
fi
